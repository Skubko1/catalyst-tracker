...
// ВАЖНО: better-sqlite3 компилирует db.prepare(...) сразу при импорте модуля,
// а это требует, чтобы таблицы уже существовали. На чистой БД (первый запуск,
// свежий раннер Actions) их ещё нет → "no such table". Поэтому создаём схему
// прямо здесь, до любых prepare ниже. CREATE IF NOT EXISTS — вызывать безопасно всегда.
initSchema();
